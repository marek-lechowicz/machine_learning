...
def read_file(file_path: str) -> str:
    ...
    try:
        ...
        file_content = ''
        print("Unable tpo open {:s}".format(file_path))
...
acc_train = np.mean((y_pred == y_train).astype(int))
...
# FIXME: Compute the training accuracy.
acc_test = np.mean((y_pred == y_test).astype(int))
...
weights = clf.coef_[0]
idx = weights.argsort()[-15:]

vocabulary_dict = sorted(get_vocabulary_dict())

print('\nTop predictors of spam: \n')
for i in range(15):
    # FIXME: Replace each `None` with an appropriate expression.
    print(' {word:<20}: {weight:10.6f}'.format(
        word=vocabulary_dict[i], weight=float(weights[i])))
